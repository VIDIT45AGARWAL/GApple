...
const FoodItem = ({dish}) => {
  ...
  // Use image_url if available, otherwise try image
  const imageUrl = getImageUrl(dish.image_url || dish.image)
  ...
  // Add a debug log to see what the dish object contains
    ...
    <>
      <div className='flex flex-col w-70 m-4 shadow-2xl rounded-2xl'>
        <div className='relative'>
          ...
          <img 
            src={imageUrl} 
            className='size-70 rounded-t-2xl object-cover' 
            alt={dish.name}
            onError={(e) => {
